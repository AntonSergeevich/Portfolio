...
def votingData(request):
    ...
    game = Question.objects.filter(answer='game').count()
    app = Question.objects.filter(answer='app').count()

   # serializer = QuestionSerializer(questions, many=True)

    return JsonResponse({'backend': backend, 'frontend': frontend, 'fullstack': fullstack, 'game': game, 'app': app})
